...
interface BulkChangeAgentModalProps {
    isOpen: boolean
    onClose: () => void
    leadIds: string[]
    onAgentChange?: () => void
...
const BulkChangeAgentModal: React.FC<BulkChangeAgentModalProps> = ({
    agentName = '',
    isOpen,
    onClose,
    leadIds,
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            // Update leads, enquiries and tasks in parallel
            await Promise.all(
                leadIds.map(async (leadId) => {
                    ...
                    const updatedAgentHistory = [...(enquiry?.agentHistory || []), newAgentHistoryEntry]
                    ...
                    await enquiryService.addActivity(enquiry.enquiryId ?? '', {
                    ...
                    const enquiryUpdatePromise = enquiryService.update(enquiry.enquiryId ?? '', enquiryUpdateData)
                    ...
                    try {
                        const allTasks = await taskService.getByEnquiryId(enquiry.enquiryId ?? '')
            ...
            console.log('Enquiries and leads updated with new agent')
            ...
            toast.success('Agents changed successfully!')

            // Reset form and close modal
            handleDiscard()
        } catch (error) {
            console.error('Error changing agent:', error)
            setError('Failed to change agents. Please try again.')
            toast.error('Failed to change agents')
        } finally {
            setIsLoading(false)
        }
    }
...
export default BulkChangeAgentModal
